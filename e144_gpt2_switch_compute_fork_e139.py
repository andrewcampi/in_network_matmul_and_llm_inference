#!/usr/bin/env python3
"""
e144_gpt2_switch_compute_fork_e139.py
...
def process_single_layer_on_switch(
    ...
    """
    Process a single transformer layer on the switch using SEQUENTIAL projection processing (e143 style).
    
    Sequential steps:
      1. Clear counters
      2. Send packets for projection
      3. Read counters
      4. Repeat for each projection
    
    This eliminates decoding ambiguity while maximizing TCAM efficiency!
    ...
    """
    print(f"\n{'='*80}")
    print(f"LAYER {layer_idx} - SEQUENTIAL PROJECTION PROCESSING")
    print(f"{'='*80}")
    ...
    # SEQUENTIAL PROCESSING: Q, K, V projections (one at a time!)
    projections = [
...
...[output truncated. This is the end of the output.]...
  ...
  FFN DOWN projection (with residual):
    Packets: 2672784 + 157632 residual = 2830416
  Using DPDK for 2830416 packets...
  DPDK output: DPDK initialized successfully!
Sending 2830416 packets...

Results:
  Packets sent: 2830416
  Time:         252.568 ms
  PPS:          11206557 (11.2M pps)
  Throughput:   5.74 Gbps

    ✓ Mean=-0.208, Std=3.113

✓ Layer 10 complete
  Layer 10 time: 62.21s

================================================================================
LAYER 11 - SEQUENTIAL PROJECTION PROCESSING
================================================================================
  ...
  Q projection:
    Packets: 14188811
  Using DPDK for 14188811 packets...
  DPDK output: DPDK initialized successfully!
Sending 14188811 packets...

Results:
  Packets sent: 14188811
  Time:         1270.415 ms
  PPS:          11168643 (11.2M pps)
  Throughput:   5.72 Gbps

    ✓ Mean=0.211, Std=1.063
...
Results:
  Packets sent: 14188811
  Time:         1275.542 ms
  PPS:          11123752 (11.1M pps)
  Throughput:   5.70 Gbps

    ✓ Mean=0.211, Std=1.063

  O projection (with residual):
    Packets: 165255 + 89088 residual = 254343
  Using DPDK for 254343 packets...
  DPDK output: DPDK initialized successfully!
Sending 254343 packets...

Results:
  Packets sent: 254343
  Time:         23.345 ms
  PPS:          10895042 (10.9M pps)
  Throughput:   5.58 Gbps

    ✓ Mean=-0.327, Std=3.171

  FFN UP projection:
    Packets: 7227891
  Using DPDK for 7227891 packets...
  DPDK output: DPDK initialized successfully!
Sending 7227891 packets...

Results:
  Packets sent: 7227891
  Time:         654.276 ms
  PPS:          11047153 (11.0M pps)
  Throughput:   5.66 Gbps

    ✓ Mean=0.735, Std=2.082

  FFN DOWN projection (with residual):
    Packets: 110193 + 36928 residual = 147121
  Using DPDK for 147121 packets...
  DPDK output: DPDK initialized successfully!
Sending 147121 packets...

Results:
  Packets sent: 147121
  Time:         13.827 ms
  PPS:          10640421 (10.6M pps)
  Throughput:   5.45 Gbps

    ✓ Mean=-0.495, Std=3.533

✓ Layer 11 complete
  Layer 11 time: 68.38s

================================================================================
ALL 12 LAYERS COMPLETE!
================================================================================
  Total time: 933.98s
  Average per layer: 77.83s
  Final hidden state: mean=-0.495, std=3.533
...
================================================================================
  ...
  Stage 1: CPU bucket-level argmax...
    Time: 98.4ms
    Winning bucket: 96/99
    Best token (preliminary): 49262
    Max logit: 50.9
  ...
  Stage 2: Switch fine-pass (simulated - using CPU result)
    ...
    Result: Same as CPU (token=49262, logit=50.9)

================================================================================
RESULT: Next token = 49262 (logit=50.9)
================================================================================
