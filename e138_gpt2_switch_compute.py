#!/usr/bin/env python3
"""
e138_gpt2_switch_compute.py

GPT-2 124M FULL TRANSFORMER BLOCK ON SWITCHES
==============================================

GOAL: Run complete GPT-2 transformer block (Attention + FFN) on switches.

Based on proven approaches:
  - e056: 4-bit weights, packet counting for matmul (99.4% correlation)
  - e057: V and O projections ✓
  - e058: FFN (gate, up, down) projections ✓
  - e059: Full transformer block (Attention + FFN) ✓
  - e074: Q@K^T attention scores ✓
  - e078: Score@V weighted sum ✓
  - e079: GQA multi-head attention ✓
  - e088: Full GPT-2 integration ✓

This implements COMPLETE TRANSFORMER BLOCK:
  1. Attention: Q, K, V, O projections
  2. FFN: UP and DOWN projections
  
Per-neuron MAC addressing:
  - Each neuron: 02:00:5e:LL:PP:NN (LL=layer, PP=projection, NN=neuron)
  - Dual counters: pos/neg for signed arithmetic
  - GGUF quantized weights (Q4_K/Q5_K/Q6_K) used directly
  - NO double quantization - use GGUF's high-quality quantization
  - Input magnitude encoded in packet counts

CRITICAL FIXES (from debugging session):
==========================================
1. DOUBLE QUANTIZATION FIX:
   Problem: GGUF loader dequantizes Q5_K→float32, then we re-quantized float32→4bit
   Impact: Destroyed FFN accuracy (U=0.42, D=0.64 correlation)
   Solution: Use GGUF's dequantized float32 directly, quantize ONCE to 4-bit range
   Result: U improved to 0.92+ correlation!

2. DEQUANTIZATION FORMULA FIX:
   Problem: Used wrong formula - DIVISION instead of correct multiply/divide
   Impact: 100× magnitude errors (Q/K/V std=100 instead of 1.0)
   Solution: result = counter * input_scale / weight_scale  [NOT divide both!]
   Explanation: Counter = sum(|W*w_scale| * |X/in_scale|) ≈ result*(w_scale/in_scale)
   Result: Magnitudes now correct, prevents scale accumulation

3. RESIDUAL QUANTIZATION FIX:
   Problem: Residual used wrong scaling (multiply when should divide, or vice versa)
   Impact: Generated 0 or wrong number of residual packets
   Solution: residual_packets = residual_value * weight_scale / input_scale
   Explanation: Match the dequant formula so residuals add correctly
   Result: Residuals now contribute correctly to final output

Test: 64d × 6 projections = 384 neurons = 768 TCAM terms (pos+neg)
Full: 768d × 6 projections × 12 layers = 55K neurons (within capacity)

This is ACTUAL neural network computation on switch hardware!
"""
...
def generate_matmul_packets(
    ...
    # Scale so max input gives ~10 packets per weight unit (adjustable)
    input_scale = max_x / 10.0
...
def main():
    """Run full transformer block test with residuals."""
    return run_full_attention()
...
""" Output:
sudo python3 e138_gpt2_switch_compute.py 
================================================================================
...
Extracting float32 weights from GGUF...
  Q weight: (64, 64), scale=7.114585
  K weight: (64, 64), scale=6.974883
  V weight: (64, 64), scale=7.914976
  O weight: (64, 64), scale=9.012101
  U weight (FFN up): (64, 64), scale=8.699854
  D weight (FFN down): (64, 64), scale=12.709352

Configuring Switch 1...
  ALL 6 projections (Q, K, V, O, U, D) - 64d
...
Verifying Switch 1 configuration...
  ✓ Switch 1: Full transformer block configured (Attention + FFN)

Creating test input...
  Input: shape=(64,), mean=0.017, std=0.507

Computing transformer block on CPU...
  Attention:
    Q: mean=0.005, std=1.001
    K: mean=0.116, std=0.992
    V: mean=0.228, std=0.946
    O (pre-residual): mean=0.074, std=0.987
    O (with residual): mean=0.090, std=1.132
  FFN:
    UP: mean=0.064, std=1.359
    DOWN (pre-residual): mean=0.018, std=0.691
    DOWN (with residual): mean=0.109, std=1.350

================================================================================
SWITCH COMPUTATION
================================================================================

Generating packets for Q, K, V projections...
  Q: 19922 packets
  K: 19273 packets
  V: 21911 packets
  Total: 61106 packets
...
Sending 61106 packets to switch...
  Using DPDK for 61106 packets...
  DPDK output: DPDK initialized successfully!
Sending 61106 packets...

Results:
  Packets sent: 61106
  Time:         5.948 ms
  PPS:          10273609 (10.3M pps)
  Throughput:   5.26 Gbps

  ✓ Sent in 5.9ms

Checking switch counters...
  Non-zero counters: 384, Total packets counted: 61106
  Sample counters:

Reading Q, K, V from switch...
  Q: mean=-0.009, std=1.017
  K: mean=0.108, std=1.041
  V: mean=0.172, std=0.978

Computing O projection with residual on switch...
  Generated 7375 packets for O projection
  Generated 946 packets for residual (FREE!)
  Total: 8321 packets
  O (with residual): mean=0.110, std=1.101

Computing FFN on Switch 1...
  Generated 10531 packets for U (FFN up)
  DEBUG: o_switch range=[-2.273, 2.827], mean=0.110
  DEBUG: scale_up=0.282683, ffn_up_scale=8.699854
  Sample U packet MAC: 02:00:5e:00:04:00
  Using DPDK for 10531 packets...
  DPDK output: DPDK initialized successfully!
Sending 10531 packets...

Results:
  Packets sent: 10531
  Time:         1.251 ms
  PPS:          8420461 (8.4M pps)
  Throughput:   4.31 Gbps

  Sent to Switch 1 (routes to SW2) in 1.3ms
  DEBUG: U counter check:
L0_U_n0_neg                                          4160                   65
L0_U_n0_pos                                          6656                  104
L0_U_n10_neg                                         1984                   31
L0_U_n10_pos                                         5120                   80
L0_U_n11_neg                                         4160                   65
L0_U_n11_pos                                         5376                   84
L0_U_n12_neg              
  DEBUG: ffn_up_switch_raw range=[-130.0, 125.0]
  DEBUG: After dequant: range=[-4.224, 4.062]
  U (FFN up): mean=0.028, std=1.360
  Generated 4477 packets for D (FFN down)
  Generated 1705 packets for residual (FREE!)
  Total: 6182 packets
  D (FFN down with residual): mean=0.118, std=1.385

================================================================================
FULL TRANSFORMER BLOCK RESULTS
================================================================================

Q Projection:
  CPU:    mean=0.005285, std=1.001195
  Switch: mean=-0.009248, std=1.017242
  Correlation: 0.9822 ✓ STRONG

K Projection:
  CPU:    mean=0.116478, std=0.991524
  Switch: mean=0.107699, std=1.041003
  Correlation: 0.9845 ✓ STRONG

V Projection:
  CPU:    mean=0.227604, std=0.946219
  Switch: mean=0.172496, std=0.978247
  Correlation: 0.9902 ✓ STRONG

O Projection:
  CPU:    mean=0.090269, std=1.131811
  Switch: mean=0.109557, std=1.101298
  Correlation: 0.9760 ✓ STRONG

U (FFN up) Projection:
  CPU:    mean=0.063977, std=1.359340
  Switch: mean=0.027924, std=1.360059
  Correlation: 0.9521 ✓ STRONG

D (FFN down) Projection:
  CPU:    mean=0.108707, std=1.349527
  Switch: mean=0.117844, std=1.384667
  Correlation: 0.9608 ✓ STRONG

================================================================================
FULL TRANSFORMER BLOCK WITH RESIDUALS - ALL ON SWITCHES!
================================================================================

✓ Switch 1: Attention (Q, K, V, O) + Residual
✓ Switch 2: FFN (U, D) + Residual
✓ Complete GPT-2 transformer block with residual connections!
✓ Residuals are FREE - packets sum automatically in counters (e070)
✓ Based on proven architectures: e057, e058, e059, e070
✓ Ready to scale to full 768d × 12 layers

Architecture:
  - Per-neuron MAC addressing: 02:00:5e:LL:PP:NN
  - Dual counters (pos/neg) for signed arithmetic
  - 4-bit quantized weights from GGUF
  - Input magnitude encoded in packet counts
  - Residual connections via packet summation (zero overhead!)
  - Two-switch architecture for complete transformer block
"""
